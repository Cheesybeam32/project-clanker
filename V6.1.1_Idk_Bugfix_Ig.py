...
class brain:
    ...
    def init(self):
        if os.path.exists("Save.npz"):
            ...
            self.Input = np.random.uniform(-0.01, 0.01, size=(Cells, Dims)).astype(np.float64)
            self.Cells = np.zeros((Cells, Dims), dtype=np.float64)
            self.Weights = np.random.uniform(-0.01, 0.01, size=(Bounds, Dims)).astype(np.float64)
            self.D_Mixer = np.random.uniform(-0.01, 0.01, size=(Dims, Dims)).astype(np.float64)
            self.C_Mixer = np.random.uniform(-0.01, 0.01, size=(Cells)).astype(np.float64)
            self.Forget = np.random.uniform(-0.01, 0.01, size=(Cells, Dims)).astype(np.float64)
